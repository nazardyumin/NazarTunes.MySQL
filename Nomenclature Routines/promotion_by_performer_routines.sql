DELIMITER |
CREATE PROCEDURE procedure_promotion_by_performer_true(IN nom_id INT)
BEGIN
    UPDATE table_nomenclatures SET promotion_by_performer = TRUE WHERE nomenclature_id = nom_id;
END |


DELIMITER |
CREATE PROCEDURE procedure_promotion_by_performer_false(IN nom_id INT)
BEGIN
    UPDATE table_nomenclatures SET promotion_by_performer = FALSE WHERE nomenclature_id = nom_id;
END |


DELIMITER |
CREATE PROCEDURE procedure_set_promotional_price_by_performer(IN id_performer INT, IN new_discount INT)
BEGIN
    ...
    DECLARE all_nom_ids CURSOR FOR SELECT nomenclature_id
                                   ...
                                   WHERE performer_id = id_performer
    ...
    LOOP
        ...
        IF @actual_price > 0 THEN
            ...
            CALL procedure_promotion_by_performer_true(nom_id);
        END IF;
    END LOOP;
    CLOSE all_nom_ids;
END |
...
DELIMITER |
CREATE PROCEDURE procedure_return_normal_price_by_performer(IN id_performer INT, IN new_discount INT)
BEGIN
    DECLARE done BOOL DEFAULT FALSE;
    DECLARE nom_id INT;
    ...
    DECLARE all_nom_ids CURSOR FOR SELECT nomenclature_id
                                   FROM table_nomenclatures
                                            ...
                                            JOIN table_record_performer_items
                                                 ON table_records.record_id = table_record_performer_items.record_id
                                   WHERE performer_id = id_performer
                                     AND promotion_by_performer = TRUE;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    OPEN all_nom_ids;
    read_loop:
    LOOP
        ...
        CALL procedure_promotion_by_performer_false(nom_id);
    END LOOP;
    CLOSE all_nom_ids;
END |
...
DELIMITER |
CREATE PROCEDURE procedure_create_new_promotion_by_performer(IN id_performer INT, IN new_discount INT, IN new_is_started BOOL)
BEGIN
    DECLARE promo_id INT;
    INSERT INTO table_discount_promotions (performer_id, discount, is_started)
    VALUES (id_performer, new_discount, new_is_started);
